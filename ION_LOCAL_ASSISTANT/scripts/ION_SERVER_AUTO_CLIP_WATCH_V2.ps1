...
Write-Host 'READY=1'
while ($true) {
  try { $Clip = Get-Clipboard -Raw -ErrorAction Stop } catch { Start-Sleep -Seconds 1; continue }
  if ($null -eq $Clip) { Start-Sleep -Seconds 1; continue }
  $Trim = $Clip.Trim()
  if ($Trim -ne $Marker) { Start-Sleep -Seconds 1; continue }
  $Bytes = [Text.Encoding]::UTF8.GetBytes($Trim)
  $Hash = [BitConverter]::ToString([Security.Cryptography.SHA1]::Create().ComputeHash($Bytes))
  if ($Hash -eq $LastHash) { Start-Sleep -Seconds 1; continue }
  $LastHash = $Hash
  ...
  Set-Content -Path (Join-Path $Root 'ION_LOCAL_ASSISTANT\storage\reports\server_auto_clip_watch_v2_last_output.txt') -Value $Out -Encoding UTF8
  Set-Clipboard -Value $Out
  Write-Host 'AUTO_DONE_OUTPUT_COPIED_TO_CLIPBOARD'
